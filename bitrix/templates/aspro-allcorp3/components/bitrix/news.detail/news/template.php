...
//    'BROCHURE' => $arResult['PROPERTIES']['BROCHURE']['VALUE'],
...
$templateData['TABS'] = [];
foreach ($mapTabs as $key => $row) if ($arResult['PROPERTIES'][$key]['VALUE']) {
    $templateData[$key] = $arResult['PROPERTIES'][$key]['VALUE'];
    $templateData['TABS'][] = strtolower($key);
    $templateData['T_' . strtoupper($key)] = $row;
    unset($arResult['PROPERTIES'][$key]);
}
$arParams["DETAIL_BLOCKS_TAB_ORDER"] .= ',' . implode(',', array_keys($mapTabs));
?>
    <style>.props_block--nbg .char_value {
            text-align: right
        }</style>
